...
<?php
	class envios
	{
		private static function print_problem_chooser( $contest_id = null )
		{
			...
			if($contest_id === null)
			{
				if (isset($_GET["send_to"])) { $presend = $_GET["send_to"]; }
			...
			<script>
				...
				function parse_the_result_from_teddy(response)
				{
					...
					switch(response.status)
					{
						...
						case "ERROR":
							html += "WHOA! Teddy tiene problemas para evaluar tu código.";
						break;
						...
						case "COMPILACION":
							html += "¡Tu programa no compiló!";
							html += "<pre style='text-align:left'>" + response.compilador + "</pre>";
						break;

						case "RUNTIME_ERROR":
							html += "Tu programa arrojo una excepción !";
						break;
					...
					<tr>
						<td colspan=1 >
							...
							<div
								...
								&iquest; Problemas? <br>Puedes intentar otra manera de subir el código <a href="#">aquí</a>.</div>
							<br><br>
						</td>
					</tr>
					<tr>
